...
function Qna() {
  ...
  async function handleDeleteSession(index) {
    ...
    const id = allSessions[index]._id;
    try {
      const resp = await axios.delete(
      `${import.meta.env.VITE_REACT_APP_API_URL}/qna/delete/${id}`,
      {
        headers: {
      ...
      if (resp.data.success) {
        ...
        alert("Failed to delete session" + resp.data.message);
      }
    } catch (error) {
        ...
        {allSessions.map((session, index) => (
          ...
          >
            ...
            <button
              ...
              className="absolute top-2 right-2 text-pink-500 hover:text-red-600"
            >
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-5 text-gray-500 cursor-pointer hover:scale-110 transition transform hover:text-red-600"
              >
                <path
                  strokeLinecap="round"
            ...
            {/* Header */}
            <div className="mb-4" onClick={() => handleSessionClick(session._id)}>
              <div
